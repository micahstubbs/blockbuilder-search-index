...
blocksStr = fs
  ...
  .toString();
let blocksLinks2 = d3.csv.parse(blocksStr);
blocksLinks2.forEach(function(d) {
  const username = parseBlockURL(d.url);
  return (userHash[username] = 1);
});
let blocksusers2 = Object.keys(userHash).length;
total += blocksusers2;
...
let csvusers = Object.keys(userHash).length - total;
total += csvusers;
...
userscsv = fs
  .readFileSync(
    'data/user-sources/github-users-with-d3-in-username-or-repo-bigquery.csv'
  )
  .toString();
d3.csv.parse(userscsv, function(user) {
  const username =
